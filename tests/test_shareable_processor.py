...
from shareable_processor import escape_applescript_string, extract_end_date
...
def test_escape_applescript_string():
    ...
    assert escape_applescript_string("mix\\\"match\"") == 'mix\\\\\\"match\\"'
    assert escape_applescript_string(None) == ""
    assert escape_applescript_string(123) == "123"
...
def test_extract_end_date_from_dataframe_fallback():
    ...
    end_date = extract_end_date(filename, df)
